...
bufferMeasures = "";
overlayPath = path + "overlay_" + threshold_method + "_correc-factor-" + correc_factor + "_dist-" + distance_threshold + "_area-" + area_threshold + "_radDilation-" + radiusDilation + "/";
...
for(cont=0; cont<dir.length; cont++) { 
	...
	if(startsWith(file, "SNI") || startsWith(file, "Neg SNI")) { // exclude the overlay folder from the analysis if it is already created
		outpath = path + file + "output_" + threshold_method + "_correc-factor-" + correc_factor + "_dist-" + distance_threshold + "_area-" + area_threshold + "_radDilation-" + radiusDilation + "/";
