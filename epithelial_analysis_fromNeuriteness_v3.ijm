...
neur_channels = newArray("Cy5");//("FITC", "cy3", "Cy5", "m cherry"); // select channels to be combined for neuriteness
					...
					if(neurit_file) {
						...
						if(countSelecArea > 0) {
							...
							if(countSelected != 0) {
								...
								area_inteact_net = 0;
								if(combine_neur) area_inteact_net = getIntactNet("", outpath);
								else area_inteact_net = getIntactNet(channel, outpath);
...
summaryPathFITC = path + "summaryFITC_" + exp_id + ".csv";
summaryPathCy3 = path + "summaryCy3_" + exp_id + ".csv";
summaryPathCy5 = path + "summaryCy5_" + exp_id + ".csv";
summaryPathMcherry = path + "summaryMcherry_" + exp_id + ".csv";
if(File.exists(summaryPathFITC))
	File.delete(summaryPathFITC);
if(File.exists(summaryPathCy3))
	File.delete(summaryPathCy3);
if(File.exists(summaryPathCy5))
	File.delete(summaryPathCy5);
if(File.exists(summaryPathMcherry))
	File.delete(summaryPathMcherry);

summaryFileFITC = File.open(summaryPathFITC);
print(summaryFileFITC, "image;number of pixels in the basal layer;average height basal layer;std basal layer;min basal layer;max basal layer;number of pixels in the apical layer;height apical layer;std apical layer;min apical layer;max apical layer;area - epithelium; area - marker of interest;MFI - marker of interest;stdMFI - marker of interest;area - neuriteness;MFI - neuriteness;stdMFI - neuriteness;relative area - marker of interest per total epithelium;realtive area - neuriteness per total epithelium;realtive area - neuriteness per marker of interest;area - marker of interest + parabasal layer;area - upper layer;area - lower layer;average height between marker of interest and apical layer;std height between marker of interest and apical layer;min height between marker of interest and apical layer;max height between marker of interest and apical layer;average height between marker of interest and basal layer;std height between marker of interest and basal layer;min height between marker of interest and basal layer;max height between marker of interest and basal layer;area - intact net;relative area - intact net per neuriteness total area\n");
print(summaryFileFITC, bufferMeasuresFITC);
File.close(summaryFileFITC);

summaryFileCy3 = File.open(summaryPathCy3);
print(summaryFileCy3, "image;number of pixels in the basal layer;average height basal layer;std basal layer;min basal layer;max basal layer;number of pixels in the apical layer;height apical layer;std apical layer;min apical layer;max apical layer;area - epithelium; area - marker of interest;MFI - marker of interest;stdMFI - marker of interest;area - neuriteness;MFI - neuriteness;stdMFI - neuriteness;relative area - marker of interest per total epithelium;realtive area - neuriteness per total epithelium;realtive area - neuriteness per marker of interest;area - marker of interest + parabasal layer;area - upper layer;area - lower layer;average height between marker of interest and apical layer;std height between marker of interest and apical layer;min height between marker of interest and apical layer;max height between marker of interest and apical layer;average height between marker of interest and basal layer;std height between marker of interest and basal layer;min height between marker of interest and basal layer;max height between marker of interest and basal layer;area - intact net;relative area - intact net per neuriteness total area\n");
print(summaryFileCy3, bufferMeasuresCy3);
File.close(summaryFileCy3);

summaryFileCy5 = File.open(summaryPathCy5);
print(summaryFileCy5, "image;number of pixels in the basal layer;average height basal layer;std basal layer;min basal layer;max basal layer;number of pixels in the apical layer;height apical layer;std apical layer;min apical layer;max apical layer;area - epithelium; area - marker of interest;MFI - marker of interest;stdMFI - marker of interest;area - neuriteness;MFI - neuriteness;stdMFI - neuriteness;relative area - marker of interest per total epithelium;realtive area - neuriteness per total epithelium;realtive area - neuriteness per marker of interest;area - marker of interest + parabasal layer;area - upper layer;area - lower layer;average height between marker of interest and apical layer;std height between marker of interest and apical layer;min height between marker of interest and apical layer;max height between marker of interest and apical layer;average height between marker of interest and basal layer;std height between marker of interest and basal layer;min height between marker of interest and basal layer;max height between marker of interest and basal layer;area - intact net;relative area - intact net per neuriteness total area\n");
print(summaryFileCy5, bufferMeasuresCy5);
File.close(summaryFileCy5);

summaryFileMcherry = File.open(summaryPathMcherry);
print(summaryFileMcherry, "image;number of pixels in the basal layer;average height basal layer;std basal layer;min basal layer;max basal layer;number of pixels in the apical layer;height apical layer;std apical layer;min apical layer;max apical layer;area - epithelium; area - marker of interest;MFI - marker of interest;stdMFI - marker of interest;area - neuriteness;MFI - neuriteness;stdMFI - neuriteness;relative area - marker of interest per total epithelium;realtive area - neuriteness per total epithelium;realtive area - neuriteness per marker of interest;area - marker of interest + parabasal layer;area - upper layer;area - lower layer;average height between marker of interest and apical layer;std height between marker of interest and apical layer;min height between marker of interest and apical layer;max height between marker of interest and apical layer;average height between marker of interest and basal layer;std height between marker of interest and basal layer;min height between marker of interest and basal layer;max height between marker of interest and basal layer;area - intact net;relative area - intact net per neuriteness total area\n");
print(summaryFileMcherry, bufferMeasuresMcherry);
File.close(summaryFileMcherry);

/****************** functions ****************************/

function getIntactNet(channel, out_path) {
