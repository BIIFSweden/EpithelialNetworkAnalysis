...
// This version of the epithelial_analysis pipeline combines the neuriteness of different input channels
// ===============================
...
neur_channels = newArray("FITC", "cy3", "Cy5"); // select channels to be combined for neuriteness
//check the intersection 
//channel = "FITC"; // select channel to be analyzed
...
function getCombinedNeuriteness(file, fileList, outPath) {
	// iterate through all channels and search for the one(s) of interest
	for(i=0; i<fileList.length; i++) {
		image = fileList[i];

		for(j=0; j<neur_channels.length; j++) {
			if(matches(image, ".*" + neur_channels[j] + ".*") && startsWith(image, "Ne_")) { // if channel of interest
				...
				rename("neuriteness_" + neur_channels[j]);

				selectWindow("mask_tissue");
				run("Create Selection");
				selectWindow("neuriteness_" + neur_channels[j]);
				run("Restore Selection");
				run("Clear Outside");
				run("Select None");

				getEpithelium(replace(file, "/", ""), getWidth(), getHeight());
				run("Create Selection");
				...
				run("Duplicate...", "mask_neuriteness_" + neur_channels[j]);
				rename("mask_neuriteness_" + neur_channels[j]);
				run("Restore Selection");
							
				setAutoThreshold(threshold_method+" dark");
				getThreshold(lower, upper);
				run("Convert to Mask");
				close("mask_neuriteness_" + neur_channels[j]);
	...
	selectWindow("neuriteness_" + neur_channels[0]);
	rename("neuriteness");
	
	for(i=0; i<(neur_channels.length-1); i++) {
		imageCalculator("Add create", "neuriteness","neuriteness_"+neur_channels[(i+1)]);
		rename("neuriteness");
		close("neuriteness_"+neur_channels[(i+1)]);
	}
	...
	if(startsWith(file, "SNI") || startsWith(file, "Neg SNI")) { // exclude the overlay folder from the analysis if it is already created
		...
		for(cont2=0; cont2<images.length; cont2++) {
			...
			if(matches(image, ".*" + channel + ".*") && startsWith(image, "Ne_")) { // check if file is an image
				...
				aux2 = substring(image, 3, lengthOf(image)); 
				...
				if(File.exists(path + file + "newTiffImages/" + aux2)) {
					...
					if(File.exists(maxProjPath + folderImg + "_AB.zip")) {
						...
						if(countRois == 2) { // then create contours of apical and basal layers and measure average height
							...
							// read original FITC channel again, segment and apply distance transform
							run("Bio-Formats Importer", "open=[" + path + file + "newTiffImages/" + aux2 + "] color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
							rename("original");
							wid = getWidth();
							hei = getHeight();
							...
							getCombinedNeuriteness(file, images, outpath);
