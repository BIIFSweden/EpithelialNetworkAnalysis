	...
	if(startsWith(file, "SNI") || startsWith(file, "Neg SNI")) { // exclude the overlay folder from the analysis if it is already created
		...
		for(cont2=0; cont2<images.length; cont2++) {
			...
			if(matches(image, ".*" + channel + ".*") && !startsWith(image, "Ne_")) { // check if file is an image
				...
				run("Bio-Formats Importer", "open=[" + path + file + "newTiffImages/" + image + "] color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
				rename("inputImg");
				...
				if(File.exists(maxProjPath + folderImg + "_AB.zip")) {
					...
					if(countRois == 2) { // then create contours of apical and basal layers and measure average height
						...
						if(File.exists(path + file + "Ne_" + image)) {
							// read corresponding neuriteness image
							run("Bio-Formats Importer", "open=[" + path + file + "Ne_" + image + "] color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
							rename("neuriteness");
							run("Duplicate...", "neuriteness_original");
							...
							run("Restore Selection");
							run("Clear Outside");
							setThreshold((lower*correc_factor), 65536);
							setOption("BlackBackground", true);
							run("Convert to Mask");
							//saveAs("Tif", overlayPath + "thresholded_" + channel + "channel" + image);
							saveAs("Tif", outpath + "thresholded_" + channel + "channel.tif");
							...
							if(countSelecArea > 0) {
								...
								selectWindow("thresholded_" + channel + "channel.tif");
								run("Invert");
								selectWindow("Mask");
								run("Select None");
								...
								// delete indexToDelete, the remaining selections are bigger than threshold
								countSelected = 0;
								for(i=0; i<nRois; i++) {
								...
								if(countSelected != 0) {
									...
									selectWindow("thresholded_" + channel + "channel.tif");
									saveOverlay("thresholded_" + channel + "channel.tif", outpath + "thresholded_segmented_" + channel + "channel.tif", false);
									saveOverlay("original", outpath + "overlay_segmentation_" + channel + "channel.tif", true);
									saveAs("Tif", overlayPath + "overlay_segmentation_" + channel + "channel_" + image);
									
									// segmentation of the neuriteness network
									selectWindow("neuriteness");
									run("Select None");
									...
									run("Measure");
									tot_area_intact_net = getResult("Area", 0);
									...
									saveAs("Tif", outpath + "thresholded_segmented_neuriteness.tif_");
									...
									run("Close");
									run("Select None");
									saveOverlay("neuriteness_original", outpath + "overlay_segmentation_neuriteness", true);
									saveAs("Tif", overlayPath + "overlay_segmentation_neuriteness_" + image);
									...
									//save in the buffer the mean intensity of the segmented channel
									selectWindow("original");
									run("Restore Selection");
									run("Measure");
							...
							run("Close All");
							print(path + file + "Ne_" + image + " does not exist");
		...
		if(dapiImg && channelImg) {
			...
			//print("total_area_parabasal: " + tot_area_parabasal);
			selectWindow("Results"); 
			run("Close");
			...
			//print("total_area_upper: " + tot_area_upper);
			selectWindow("Results"); 
			run("Close");
			...
			//print("total_area_upper: " + tot_area_lower);
			selectWindow("Results"); 
			run("Close");
